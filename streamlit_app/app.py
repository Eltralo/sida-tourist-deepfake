...
Демо дообученной мультимодальной модели SIDA-7B
...
# Подключение общие модулей streamlit_app/ к sys.path
HERE = Path(__file__).resolve().parent
sys.path.insert(0, str(HERE))

# Корень репозитория — на уровень выше streamlit_app/.
# Использование для дефолтных путей к ck/ и для импортов model.* / utils.*
